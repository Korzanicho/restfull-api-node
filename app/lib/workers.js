...
workers.validateCheckData = function (originalCheckData) {
  ...
  originalCheckData.userPhone = typeof (originalCheckData.userPhone) == 'string' && originalCheckData.userPhone.trim().length == 9 ? originalCheckData.userPhone : false;
  ...
  workers.performCheck(originalCheckData);
    ...
    console.log("Error: One of the checks is not property formatted. Skipping it");
  }
}
...
workers.performCheck = function (originalCheckData) {
  ...
  var path = parsedUrl.path; // Using path and not pathname bcs we want the query string
...
workers.log = function(originalCheckData ,checkOutcome, state, alertWarranted, timeOfCheck) {
  ...
  _logs.append(logFileName, logString, function (err) {
    if (!err) {
      console.log("Logging to file successes");
    } else {
      console.log("Logging to file filed");
    }
  });
};
...
// Timer to execute the log-rotation process onece per day
